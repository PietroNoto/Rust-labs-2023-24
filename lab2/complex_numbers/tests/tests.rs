...
pub fn test_try_form_f64() {
    ...
    // and write test
    let f = 3.0;
    let c = ComplexNumber::try_from(f);
    assert_eq!(c.unwrap().real(), f);
}
...
pub fn test_hash_with_hash_map() {
    ...
    let a = ComplexNumber::new(1.11, 2.22);
    let b = ComplexNumber::new(2.22, 4.44);
...
pub fn test_deque() {
    ...
    let mut dq = VecDeque::<ComplexNumber>::with_capacity(10);
    for i in 0..5
    {
        dq.push_back(ComplexNumber::new(i as f64, i as f64));
    }
    for i in 5..10
    {
        dq.push_front(ComplexNumber::new(i as f64, i as f64));
    }
    let mut index = dq.binary_search(&ComplexNumber::new(2 as f64, 2 as f64));

    dq.make_contiguous().sort();

    index = dq.binary_search(&ComplexNumber::new(2 as f64, 2 as f64));
